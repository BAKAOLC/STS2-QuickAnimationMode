...
namespace STS2QuickAnimationMode.Patches
{
    internal static class HandLayoutStabilizer
    {
        ...
        {
            if (!Main.IsModActive || TargetAngleField == null || AngleCancelTokenField == null)
                return false;
        ...
        {
            if (!Main.IsModActive || TargetPositionField == null || PositionCancelTokenField == null)
                return false;
        ...
        {
            if (!Main.IsModActive || TargetScaleField == null || ScaleCancelTokenField == null)
                return false;
        ...
        private static async Task AnimatePosition(NHandCardHolder holder, Vector2 targetPosition, CancellationTokenSource token)
        {
            while (!token.IsCancellationRequested && holder.IsValid())
            {
    ...
    public class HandTargetPositionStabilizationPatch : IPatchMethod
    {
        ...
        {
            return [new(typeof(NHandCardHolder), nameof(NHandCardHolder.SetTargetPosition), [typeof(Vector2)])];
        }
    ...
    public class HandTargetAngleStabilizationPatch : IPatchMethod
    {
        ...
        public static ModPatchTarget[] GetTargets()
        {
            return [new(typeof(NHandCardHolder), nameof(NHandCardHolder.SetTargetAngle), [typeof(float)])];
        }
    ...
    public class HandTargetScaleStabilizationPatch : IPatchMethod
    {
        ...
        public static ModPatchTarget[] GetTargets()
        {
            return [new(typeof(NHandCardHolder), nameof(NHandCardHolder.SetTargetScale), [typeof(Vector2)])];
        }
