...
namespace STS2QuickAnimationMode.Patches
{
    internal static class HandLayoutStabilizer
    {
        ...
        private static async Task AnimateScale(NHandCardHolder holder, Vector2 targetScale,
            ...
            CancellationTokenSource token)
        {
            ...
            if (holder.IsValid()
                ...
                && holder.Position.DistanceSquaredTo(targetPosition) <
                ReenableHitboxThreshold * ReenableHitboxThreshold)
                holder.Hitbox.SetEnabled(true);
        }
    ...
    public class HandTargetPositionStabilizationPatch : IPatchMethod
    {
        ...
        public static ModPatchTarget[] GetTargets()
        {
            return
            [
                new(typeof(NHandCardHolder), nameof(NHandCardHolder.SetTargetPosition), [typeof(Vector2)]),
            ];
        }
